USE ROLE SYSADMIN;
USE DATABASE STITCH;
USE SCHEMA SALESFORCEFSL3;
USE WAREHOUSE ACCOUNTADMIN_MGMT;

CREATE OR REPLACE PROCEDURE STITCH.SALESFORCEFSL3.CREATE_DYNAMIC_VIEW(db_name string, schema_name string, table_name string)
RETURNS STRING
LANGUAGE PYTHON
RUNTIME_VERSION = '3.10'
PACKAGES = ('snowflake-snowpark-python', 'pandas')
HANDLER = 'main'
COMMENT = 'Created by Luis Fuentes, this recives a db_name, schema_name and table_name and creates a view witht the same name as the table_name + adding _v and casts all VARCHAR columns to 1000 max length'
EXECUTE AS CALLER
AS
$$
import pandas as pd
...
CALL STITCH.SALESFORCEFSL3.CREATE_DYNAMIC_VIEW('STITCH', 'SALESFORCEFSL3', 'ACCOUNT');
